...
public class CommandBlob implements Command {
    ...
    public Stream<String> autoCompleteOptions(Cmdr cmdr, String[] cmdParts, Object[] extraArgs) {
        ...
        Stream<String> a = Stream.empty();
        if (cmdParts.length - 1 - i[0] < cmd.parameters.stream().filter(x -> x.required).count() && cmdParts.length - 1 - i[0] >= 0)
            a = Arrays.stream(cmd.parameters.get(cmdParts.length - 1 - i[0]).autoCompleteOptions)
                    .map(cmdr::prefixAutofillOption);
        Stream<String> b = Stream.empty();
        if (cmdParts.length - 1 - i[0] >= 0) {
            CommandParameter<?> parameter = cmd.parameters.get(cmdParts.length - 1 - i[0]);
            if (parameter.autoCompleteDelegate != null)
                b = parameter.autoCompleteDelegate.autoInvoke(extraArgs);
        }
